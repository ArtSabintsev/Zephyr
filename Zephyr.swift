...
public class Zephyr: NSObject {
    ...
    /**
     ...
     Optional delegate for Zephyr.
     
     */
    weak var delegate: ZephyrDelegate?
    ...
    func syncFromCloud(key key: String? = nil, value: AnyObject? = nil) {
        ...
        guard let key = key else {
            for (key, value) in zephyrRemoteStoreDictionary {
                ...
                delegate?.zephyrDidSyncFromCloudToLocal(value, forKey: key)
        ...
        if let value = value {
            ...
            delegate?.zephyrDidSyncFromCloudToLocal(nil, forKey: key)
