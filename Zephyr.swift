...
public class Zephyr: NSObject {
    ...
    /**
     ...
     - parameter keys: If you pass in one or more keys, only those keys will be synchronized. If no keys are passed, then all NSUserDefaults will be synchronized with NSUbiquitousKeyValueStore.
     */
    public static func sync(_ keys: String...) {
        if keys.count > 0 {
            sync(keys)
            return
        }
     ...
     */
    public static func addKeysToBeMonitored(_ keys: [String]) {
        for key in keys {
     ...
     */
    public static func removeKeysFromBeingMonitored(_ keys: [String]) {
        for key in keys {
...
extension Zephyr {
     ...
     */
    private func registerObserver(_ key: String) {
     ...
     */
    private func unregisterObserver(_ key: String) {
        if key == ZephyrSyncKey {
            return
        }
